package sin.sin2017.project;

public interface Constants {

    public static final String LIGHT_BLACKBOARD_ID = "20";
    public static final String LIGHT_MIDDLE_ID = "21";
    public static final String LIGHT_WINDOW_ID = "22";
    public static final String LIGHT_TURN_ON = "On";
    public static final String LIGHT_TURN_OFF = "Off";

    public static final String BLIND_ID = "13";
    public static final String PROJECTOR_ID = "36";
    public static final int BLIND_CLOSE_ON_PROJECTOR_ON = 20;

    public static final String TIME_ELEMENT_ID = "27";

    public static final String MOTION_SENSOR_ID = "32";

    public static final String TEMPERATURE_SENSOR_ID = "24";

    public static final String LIGHT_LEVEL_SENSOR_ID = "31";
    public static final int LIGHT_LEVEL_TO_MUCH_SUN = 50;
    public static final int LIGHT_LEVEL_TO_LOW_SUN = 20;

    public static final String NON_REPLY = "no-reply";
}
